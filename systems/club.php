...
require "check_login.php";$admin_id=$_GET["admin_id"];
...
//}  
//else{
$cmd=mysqli_query($con,"insert into tbl_club (club_id,id_liga,name,nickname,establish_date,phone,fax,email,stadium,competition,address,description,logo,IDProvinsi,IDKabupaten,coach,manager,website,stadium_capacity,stadium_address,owner,supporter_name,training_schedule,alumnus_name,legalitas_pt,legalitas_kemenham,legalitas_npwp,legalitas_dirut,url,admin_id) values ('$nclub','".$_POST["id_liga"]."','$name','$nickname','$establish_date','$phone','$fax','$email','$stadium','$competition','$address','$description','$logo','".$_POST["IDProvinsi"]."','".$_POST["IDKabupaten"]."','".$coach."','".$manager."','".$website."','".$stadium_capacity."','".$stadium_address."','".$owner."','".$supporter_name."','".$training_schedule."','".$alumnus_name."','".$legalitas_pt."','".$legalitas_kemenham."','".$legalitas_npwp."','".$legalitas_dirut."','".$url."','".$admin_id."')");
...
$result=mysqli_query($con,"SELECT a.*,b.username,c.fullname as admin_name
FROM tbl_club a 
LEFT JOIN tbl_users b ON b.user_id=a.user_id
LEFT JOIN tbl_admin c on c.admin_id = a.admin_id
order by a.club_id desc");
$i=1;
